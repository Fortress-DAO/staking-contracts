...
contract FortTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SFORT }

    address public immutable FORT;
    ...
    address public sFORT;
    uint public sFORTQueue; // Delays change to sFORT address
    ...
    constructor (
       ...
       /* isLiquidityToken[ _FORTMIM ] = true;
       liquidityTokens.push( _FORTMIM ); */

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for FORT
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint FORT needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( FORT ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn FORT for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IFORTERC20( FORT ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sFORT ).balanceOf( msg.sender ); // Can only borrow against sFORT held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with FORT
        @param _amount uint
     */
    function repayDebtWithFORT( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IFORTERC20( FORT ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, FORT, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( FORT ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( FORT ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns FORT valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match FORT decimals
            value_ = _amount.mul( 10 ** IERC20( FORT ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SFORT ) { // 9
            sFORTQueue = block.number.add( blocksNeededForQueue );
        } else return false;
